    ...
    for t in range(10):
        ...
        while (torch.linalg.norm(beta - beta_previous) > 1e-4 and iteration < 20):
            ...
            for j in range(0, P):
                ...
                C2 = 1 / b + (beta.abs().pow(0.5 ** s)).sum() - beta[j,:].abs().pow(0.5 ** s)
                
                if Z[j].abs() < 2 * ((C1 / (C2 + Z[j].abs().pow(0.5 ** s))) * 0.5 / XTX[j,j]).pow(power):
                    beta[j,:] = 0
                else:
                    beta_old = Z[j].abs()
                    ...
                    while ((beta_old - beta_new).abs() > 1e-4 and k <= 20 and beta_new >= 0):

                        beta_old = torch.clone(beta_new)
                        beta_new = Z[j].abs()-C1 / XTX[j,j] / (beta_old + C2 * beta_old.pow(1 - 0.5 ** s))
                        k = k + 1

                    if k > 20 or beta_new < 0:
                        beta[j,:] = 0 
                    else:
                        beta[j,:] = beta_new * Z[j].sign()
...
def SQR(Y, X, Q = 0.5, C = 0.5, s = 1, path = False):
    ...
    for t in range(0, iteration):
        ...
        L1_error = torch.maximum((Y - X @ beta).abs(),torch.tensor(0.1, device = device))
        T = Y - (1 - 2 * Q) * L1_error
            ...
            for j in range(0, P):
                ...
                if Z[j].abs() <= 2* (C1 / (2 * C2 + 2 * Z[j].abs().pow(0.5 ** s)) / XTWX[j]).pow(power):
                    ...
                    while((beta_old - beta_new).abs() > 1e-4 and k < 20 and beta_new >= 0):
                        ...
                        beta_old = torch.clone(beta_new)
                        beta_new = Z[j].abs() - C1 / XTWX[j] / (beta_old + C2 * beta_old.pow(1 - 0.5 ** s))
                        k = k + 1
