...
def BQR(Y,X,alpha,a,b,Q = 0.5,M = 10000,burn_in = 10000):
    ...
    for i in tqdm(range(1,M+burn_in)):
        ...
        if P > N:
            ...
            beta_sample = torch.linalg.solve(GXTD@GXTD.T+torch.eye(P,device=device,dtype = torch.float64),GXTD@DY+GXTD@torch.randn_like(Y)+torch.randn_like(beta_sample))*G
        
        #Sample omega
        omega_sample = (inv_gauss(2/torch.maximum(torch.abs((Y-X@beta_sample).ravel()),torch.tensor(1e-3,device=device,dtype = torch.float64)))/(2*Q*(1-Q))).view(-1,1)
